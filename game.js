(() => {
  ...
  const IMAGE_ASSETS = {
    ...
    stageDaghestanAlt: "./assets/stage-daghestan-2-9-16.png",
  ...
  const EVENTS = [
    ...
    },
    {
      ...
      campOnly: true,
      options: [
        { label: "Prendre la semaine debout", tag: "Striking", result: "Votre distance devient plus nette et les entrees adverses se paient plus cher.", load: 2, risk: 6, effects: { money: -5000, condition: -3, stats: { striking: 5, power: 1, grappling: -1 } } },
        { label: "Refuser le stage", tag: "Refus", result: "Vous ne partez pas sur ce tunnel debout. Le camp reprend son fil normal: choisissez l'entrainement de la semaine.", summary: "Aucun effet immediat.", load: 0, risk: 0, effects: {}, skipOpportunity: true },
      ],
    },
    {
      id: "dagestan-years",
      title: "Deux ou trois ans au Daghestan",
      text: "Un coach de sambo vous propose une immersion de montagne: lutte, sorties de cage et controle au sol.",
      campOnly: true,
      minTier: 1,
      options: [
	        { label: "Accepter la montagne", tag: "Sambo", result: "Le corps prend cher, mais votre controle cage-sol change de densite.", load: 4, risk: 13, effects: { money: -14000, morale: -3, condition: -8, injuryRisk: 5, stats: { wrestling: 5, grappling: 4, discipline: 2, striking: -2 } } },
	        { label: "Refuser le stage", tag: "Refus", result: "Vous refusez l'immersion. Le camp reprend son fil normal.", summary: "Aucun cout. Aucun effet immediat.", load: 0, risk: 0, effects: {}, skipOpportunity: true },
	      ],
	    },
    {
      id: "forest-camp",
    ...
    },
    {
      ...
      text: "Votre ceinture FLC attire une autre categorie. Votre manager voit une legende en deux ceintures.",
      minTier: 5,
      requiresCurrentTitle: true,
      options: [
  ...
  function clearContractObjectiveText(value = "") {
    const raw = String(value || "").trim();
    if (!raw) return "Objectif: saison gagnante et dossier propre";
    if (raw.startsWith("Objectif:")) return raw;
    if (/confiance/i.test(raw)) return "Objectif: 3 victoires propres, 0 forfait, 0 blessure majeure";
    if (/title eliminator si 2 victoires/i.test(raw)) return "Objectif: 2 victoires pour un title eliminator";
    if (/contender serie si 2 victoires/i.test(raw)) return "Objectif: 2 victoires pour une contender series";
    if (/title eliminator garanti/i.test(raw)) return "Objectif: prochain combat a forte valeur sportive";
    if (/top 15/i.test(raw)) return "Objectif: saison gagnante pour entrer top 15";
    if (/progression ranking acceleree/i.test(raw)) return "Objectif: saison gagnante pour ranking accelere";
    if (/progression ranking/i.test(raw)) return "Objectif: saison gagnante pour grimper au ranking";
    if (/activation bonus/i.test(raw)) return "Objectif: obtenir un main event pour bonus sponsor";
    if (/libre apres deux combats/i.test(raw)) return "Objectif: 2 combats puis renegociation libre";
    if (/route double champion/i.test(raw)) return "Objectif: title shot rapide vers une deuxieme ceinture";
    if (/affiches de legende/i.test(raw) || /statut legende/i.test(raw)) return "Objectif: battre des legendes et marquer le Pantheon";
    return `Objectif: ${raw}`;
  }
  ...
  function normalizeContractOffer(offer) {
    ...
    return {
      ...offer,
      titleClause: clearContractObjectiveText(offer.titleClause),
    };
  }
  ...
  function visualVariantKey(baseKey, variants = [], seed = "") {
    const keys = [baseKey, ...variants].filter(Boolean);
    if (keys.length <= 1) return keys[0] || baseKey;
    return keys[hashSeed(String(seed || baseKey)) % keys.length];
  }
  ...
  function withCareerDefaults(career) {
    ...
    career.moments = career.moments || [];
      ...
      if ((career.contract.signedYear < career.year || career.contract.tier < career.tier) && career.contract.remainingFights > 0) {
        career.contract.remainingFights = 0;
	      ...
	      career.contract.titleClause = clearContractObjectiveText(career.contract.titleClause);
	      const clauseNeedsWins = /contender|title eliminator|2 victoires/i.test(career.contract.titleClause || "");
    ...
    career.pendingContracts = Array.isArray(career.pendingContracts)
      ? career.pendingContracts.map(normalizeContractOffer)
      : null;
    ...
    if (
      ...
      contractNegotiationReady(career) &&
      contractOffersNeedRefresh(career)
    ) {
      const repairedOffers = buildContractOffers(career);
      if (repairedOffers.length) career.pendingContracts = repairedOffers;
    }
    return career;
  }
  ...
  function recoverPhase(career) {
    ...
    if ((career.season.fightsDone || 0) >= (career.season.fightsTarget || 1)) return "season-summary";
  ...
  function chooseGym(index) {
    ...
    startSeason();
	  ...
	  function eventEligible(event, career) {
	    ...
	    if (event.requiresCurrentTitle && !hasCurrentTierTitle(career)) return false;
    ...
    if (effects.length) {
	  ...
	  function chooseSeasonPauseOption(choice) {
	    ...
	    if (choice === "pause") {
	      ...
	      settleSeason(career);
	      career.pendingContracts = buildContractOffers(career);
	      showDecisionResult(career, {
  ...
  function campOpportunityVisualKey(opportunity, career = ui.career) {
    ...
    if (opportunity.id === "dagestan-years") {
      return visualVariantKey(
        "stageDaghestan",
        ["stageDaghestanAlt"],
        `${career?.seed || ""}|${career?.year || ""}|${career?.pendingFight?.opponent?.name || ""}|${career?.camp?.week || ""}|${opportunity.id}`
  ...
  function contractSigningVisualKey(career = ui.career, context = "") {
    return visualVariantKey(
      "contractSigning",
      ["contractSigningAlt"],
      `${career?.seed || ""}|${career?.year || ""}|${career?.season?.fightsDone || 0}|${context}`
    );
  }
  ...
  function visualResultConfig(key) {
    ...
    const visualResults = {
      ...
      pressPique: {
        className: "press-pique-result-card",
        image: assetUrl("pressPique"),
        alt: "Conference de presse tendue apres une attaque personnelle",
        icon: "message-circle-warning",
        kicker: "Attaque personnelle",
      },
      pressBrawl: {
        className: "press-brawl-result-card",
        image: assetUrl("pressBrawl"),
        alt: "Bousculade pendant un face-off de conference de presse",
        icon: "shield-alert",
      ...
      stageStrikingAlt: {
        className: "stage-striking-alt-result-card",
        image: assetUrl("stageStrikingAlt"),
        alt: "Stage de striking MMA alternatif",
        icon: "target",
        kicker: "Stage striking",
      },
      stageDaghestan: {
      ...
      stageDaghestanAlt: {
        className: "stage-daghestan-alt-result-card",
        image: assetUrl("stageDaghestanAlt"),
        alt: "Stage de sambo et lutte en montagne",
        icon: "mountain-snow",
        kicker: "Stage au Daghestan",
      },
      contractSigning: {
      ...
      contractSigningAlt: {
        className: "contract-signing-alt-result-card",
        image: assetUrl("contractSigningAlt"),
        alt: "Negociation et signature de contrat MMA",
        icon: "file-pen-line",
        kicker: "Signature de contrat",
      },
      trainingInjury: {
  ...
  function campOpportunityById(id) {
    return EVENTS.find(event => event.campOnly && event.id === id);
	  ...
	  function campOpportunitySwipeSummary(opportunity, career = ui.career) {
	    if (!opportunity) return "";
	    if (opportunity.id === "dagestan-years") return opportunity.text;
	    return `${opportunity.text} Banque: ${formatMoney(career.money)}.`;
	  }
  ...
  function currentTierTitle(career) {
    const currentWeight = career?.weight?.id || "";
    return (career?.titles || []).find(title => {
      if (title.tier !== career.tier || title.lost) return false;
      const titleWeight = title.weightId || title.weight?.id || "";
      if (!titleWeight) return true;
      return !currentWeight || titleWeight === currentWeight;
    }) || null;
  }
  ...
  function activeTitleCategories(career, minTier = 5) {
    const seen = new Set();
    (career?.titles || []).forEach((title, index) => {
      if (title.lost || Number(title.tier || 0) < minTier) return;
      const weightKey = title.weightId || title.weight?.id || title.weightLabel || `legacy-${index}`;
      seen.add(`${title.tier}:${weightKey}`);
    });
    return seen.size;
  }

  function tagActiveCurrentTitlesWithWeight(career) {
    const weight = career?.weight || WEIGHTS[2];
    (career?.titles || []).forEach(title => {
      if (title.tier === career.tier && !title.lost && !title.weightId) {
        title.weightId = weight.id;
        title.weightLabel = weight.label;
      }
    });
  }

  function weightMoveOptions(career) {
    const currentId = career?.weight?.id || WEIGHTS[2].id;
    const index = Math.max(0, WEIGHTS.findIndex(weight => weight.id === currentId));
    return [
      { offset: -1, label: "Descendre", weight: WEIGHTS[index - 1] },
      { offset: 1, label: "Monter", weight: WEIGHTS[index + 1] },
    ].filter(item => item.weight);
  }

  function titleRecordForCurrentCareer(career, belt) {
    return {
      tier: career.tier,
      label: belt,
      year: career.year,
      defenses: 0,
      lost: false,
      weightId: career.weight?.id || "",
      weightLabel: career.weight?.label || "",
    };
  }

  function handleTitleWin(career, fight, opponent, heldTitleBefore) {
    if (!fight.title) return;
    ...
    if (heldTitleBefore) {
      ...
      career.titles.push(titleRecordForCurrentCareer(career, belt));
      career.moments.push(`${belt} remportee contre ${opponent.name}.`);
    }
    if (career.tier === 5 && career.flags?.doublePath && activeTitleCategories(career, 5) >= 2) {
      if (!career.flags.doubleChamp) {
        career.flags.doubleChamp = true;
        career.moments.push("Double champion FLC: deux categories, deux ceintures.");
        addNews(
          career,
          "Double champion FLC",
          `${career.name} possede maintenant deux ceintures FLC. Le statut Legende est ouvert.`,
          "good"
        );
        career.flags.legendUnlockedThisFight = true;
      }
  ...
  function noteContenderClauseProgress(career, won) {
    ...
    if (contract.contenderWins >= contract.contenderWinsRequired) {
      ...
      addNews(
        career,
        "Objectif contrat valide",
        `${career.name} valide ${contract.contenderWinsRequired} victoires d'objectif. Le manager peut exiger une marche sportive superieure au prochain bilan.`,
        "good"
      );
    }
  }
  ...
  function promotionStatus(career) {
    ...
    const lastLoggedFight = fightLog[fightLog.length - 1] || null;
    const lastFightWon = Boolean(career.lastResult?.won || lastLoggedFight?.result === "Victoire");
    const winRate = honored ? wins / honored : lastFightWon ? 1 : 0;
    ...
    const contractStillRunning = (career.contract?.remainingFights || 0) > 0 && !clauseReady && !hasBelt;
    const seasonalWinSignal = lastFightWon || perfectSeason || strongSeason || career.streak > 0 || wins >= 2;
    const internationalDominance = ufcStep && !missed && seasonalWinSignal && (
      perfectSeason ||
      strongSeason ||
      wins >= 3 ||
      career.streak >= 3 ||
    ...
    const nationalBreakout = internationalStep && !missed && seasonalWinSignal && (
      perfectSeason ||
      strongSeason ||
      unbeatenRun ||
      career.streak >= 3 ||
    ...
    const localBreakout = career.tier < 2 && !missed && seasonalWinSignal && (
      perfectSeason ||
      strongSeason ||
      unbeatenRun ||
      career.streak >= 2 ||
    ...
    const ufcBreakout = ufcStep && !hasBelt && !missed && seasonalWinSignal && (
      perfectSeason ||
      strongSeason ||
      wins >= 3 ||
      career.streak >= 3 ||
    ...
    const ufcEligible = ufcStep && !missed && seasonalWinSignal && (hasBelt || internationalDominance || ufcBreakout);
    const flcTitleCategories = activeTitleCategories(career, 5);
    const doubleChampionReady = Boolean(career.flags?.doubleChamp) || flcTitleCategories >= 2;
    const doubleCategoryMoves = weightMoveOptions(career);
    const doubleCategoryEligible = legendStep && hasBelt && !doubleChampionReady && !missed && seasonalWinSignal && doubleCategoryMoves.length > 0 && (
      titleDefenses >= 1 ||
      perfectSeason ||
      (strongSeason && career.streak >= 4) ||
      career.streak >= 5 ||
      ufcWins >= 4 ||
      career.rank <= 1
    );
    const legendOvrReady = overall(career) >= 99 || doubleChampionReady;
    const ufcDominance = (
      perfectSeason ||
      strongSeason ||
      career.streak >= 5 ||
    ...
    const legendAccess = doubleChampionReady || (hasBelt && career.rank <= 1 && ufcWins >= 7);
    const legendEligible = legendStep && legendOvrReady && legendAccess && !missed && seasonalWinSignal && ufcDominance;
    const promotionBreakout = localBreakout || nationalBreakout || internationalDominance || ufcBreakout || hasBelt || clauseReady;
    const activeContractBlock = contractStillRunning && !promotionBreakout;
    const reliabilityBlock = missed > 0 || activeContractBlock;
  ...
  function contractNegotiationReady(career, status = promotionStatus(career)) {
    return Boolean(
      ...
      status.lastFightWon ||
      status.seasonalWinSignal ||
      status.promotionEligible ||
      status.doubleCategoryEligible ||
      status.localBreakout ||
      status.internationalDominance ||
      status.ufcBreakout ||
  ...
  function promotionRadar(career) {
    ...
    const radarPoint = target <= 3 ? 2 : 3;
    if (done < Math.min(radarPoint, target) || done >= target) return null;
    ...
    if (status.internationalStep && (status.nationalBreakout || status.strongSeason || status.perfectSeason || career.streak >= 3)) {
      return {
        ...
        title: "KSW/PFL surveille ton profil",
        text: "Une organisation internationale a les yeux sur toi. Si tu termines la saison avec au moins 3 victoires propres, ou invaincu, KSW/PFL peut arriver au bilan.",
        checks: ["3 victoires propres ou saison invaincue", "Aucun forfait sur la saison", cleanPoint],
        tone: "hot",
      };
    }

    if (status.legendStep && status.doubleCategoryEligible) {
      return {
        id: `double-belt-${season.year}`,
        title: "Deuxieme ceinture en vue",
        text: "Ton regne FLC ouvre une bascule de categorie. Au bilan, ton manager pourra viser une deuxieme ceinture sans effacer ta carriere actuelle.",
        checks: ["Choisir monter ou descendre", "Gagner le title shot rapide", cleanPoint],
        tone: "hot",
      };
    }

    if (status.ufcStep && (status.hasBelt || status.internationalDominance || status.ufcBreakout || status.strongSeason || career.streak >= 3 || career.rank <= 6)) {
      const title = status.hasBelt ? "Le FLC regarde ta ceinture" : "Le FLC observe ton profil";
      const text = status.hasBelt
        ? "Le FLC veut une preuve de champion: defends la ceinture ou termine fort, sans forfait ni scandale majeur."
        : "Le FLC commence a regarder. Une saison forte, 3 victoires propres, un top 5 ou une grosse serie peut declencher une offre au bilan.";
      return {
        id: `flc-${season.year}`,
        title,
        text,
        checks: status.hasBelt
          ? ["Defense ou saison dominante", "Rester gagnant", cleanPoint]
          : ["Saison forte ou 3 victoires propres", "Aucun forfait sur la saison", cleanPoint],
        tone: "hot",
      };
    }
  ...
  function chooseFight(index) {
    ...
    showDecisionResult(career, {
      ...
      visual: contractSigningVisualKey(career, `fight-${fight.opponent?.name || ""}`),
  ...
  function styleFromOnline(value = "") {
    ...
    const orgLabel = row.org || snapshot.org || snapshot.orgLabel || "";
    const rawOrgTier = row.org_tier ?? row.orgTier ?? snapshot.orgTier ?? snapshot.org_tier ?? snapshot.tier;
    const labelOrgTier = orgTierFromLabel(orgLabel);
    const orgTier = clamp(
      Number.isFinite(Number(rawOrgTier)) ? Number(rawOrgTier) : (labelOrgTier ?? 0),
      0,
      ORGS.length - 1
    );
    return {
      ...
      record,
      orgTier,
	      org: migrateOrgLabel(orgLabel || "Organisation", orgTier),
    ...
    const orgLabel = row.org || snapshot.org || snapshot.orgLabel || "";
    const rawOrgTier = row.org_tier ?? row.orgTier ?? snapshot.orgTier ?? snapshot.org_tier ?? snapshot.tier;
    const fallbackTier = clamp(
      Number.isFinite(Number(rawOrgTier)) ? Number(rawOrgTier) : (orgTierFromLabel(orgLabel) ?? 0),
      0,
      ORGS.length - 1
    );
	    ...
	    if (belts.length) {
	      return belts.slice(0, 8).map(belt => {
	        const tier = clamp(Number(belt.tier ?? fallbackTier), 0, ORGS.length - 1);
	        return {
    ...
    const orgLabel = row.org || snapshot.org || "";
    const rawTier = row.org_tier ?? row.orgTier ?? snapshot.orgTier ?? snapshot.org_tier ?? snapshot.tier;
    const labelTier = orgTierFromLabel(orgLabel);
	    const tier = clamp(Number.isFinite(Number(rawTier)) ? Number(rawTier) : (labelTier ?? 0), 0, ORGS.length - 1);
	    const org = orgForTier(tier);
  ...
  function finalizeLiveFight(career) {
    ...
    const fightOrg = orgForTier(career.tier);
    const fightOrgLabel = fightOrg.label || career.org?.label || "";
    ...
    if (won) {
      ...
      if (fightOrg.charismaWin) {
        career.stats.charisma = clamp((career.stats.charisma || 50) + fightOrg.charismaWin, 1, statCapForCareer(career));
      }
      if (method === "KO" || method === "TKO") career.record.ko += 1;
      else if (method === "Soumission") career.record.sub += 1;
      else career.record.dec += 1;
      handleTitleWin(career, fight, opponent, heldTitleBefore);
      {
    ...
    if (won && fightOrg.charismaWin) {
      analysis = [`Victoire FLC: charisme +${fightOrg.charismaWin}, la lumiere media change d'echelle.`, ...analysis].slice(0, 5);
    }
    if (career.flags?.legendUnlockedThisFight) {
      analysis = [
        "Legende de l'octogone: nouveau panel de combattants, nouveaux defis et plafond de stats ouvert.",
        ...analysis,
      ].slice(0, 5);
      career.flags.legendUnlockedThisFight = false;
    }
  ...
  function promotionContractOffer(career, org, status) {
    ...
    if (org.id === "legend") {
      return {
        ...
        fights: 5,
        purseBoost: 1.62,
        titleClause: "Objectif: battre des legendes et marquer le Pantheon",
    ...
    if (org.id === "ksw") {
      return {
        ...
        purseBoost: 1.12,
        titleClause: "Objectif: 2 victoires pour un title eliminator",
    ...
    if (org.id === "pfl") {
      return {
        ...
        purseBoost: 1.24,
        titleClause: "Objectif: 2 victoires pour une contender series",
        contenderWinsRequired: 2,
        entryRank: 18,
    ...
    if (org.id === "regional" || org.id === "national") {
      return {
        ...
        titleClause: "Objectif: saison gagnante pour ranking accelere",
        contenderWinsRequired: 0,
    ...
    return {
      ...
      label: "Signer au FLC",
      tag: "Sommet mondial",
      summary: status.hasBelt
        ? "Votre ceinture internationale ouvre la porte. Bourse de base plus basse, mais chaque victoire FLC fait bondir hype et charisme."
        : "Votre domination en KSW/PFL force le contact. Bourse de base plus basse, mais chaque victoire FLC fait bondir hype et charisme.",
      money: Math.round((46000 + hype * 360 + charisma * 180) / 1000) * 1000,
      purseBoost: 0.96,
      titleClause: status.hasBelt ? "Objectif: defendre ton statut et entrer top 15" : "Objectif: saison gagnante pour entrer top 15",
      contenderWinsRequired: 0,
      entryRank: 15,
  ...
  function doubleCategoryContractOffer(career, move, status) {
    const hype = career.hype || 0;
    const rep = career.rep || 0;
    const charisma = career.stats.charisma || 50;
    const weight = move.weight;
    const directionLabel = move.offset > 0 ? "Monter" : "Descendre";
    return {
      id: `flc-double-${move.offset > 0 ? "up" : "down"}`,
      label: `${directionLabel} en ${weight.label}`,
      tag: "Double ceinture",
      summary: `Votre regne FLC ouvre une deuxieme categorie. Votre carriere actuelle reste intacte: vous partez chercher une nouvelle ceinture en ${weight.label}.`,
      tier: 5,
      orgId: "ufc",
      money: Math.round((98000 + hype * 520 + rep * 360 + charisma * 160) / 1000) * 1000,
      fights: 3,
      purseBoost: 1.08,
      titleClause: "Objectif: title shot rapide vers une deuxieme ceinture",
      contenderWinsRequired: 0,
      entryRank: 3,
      sponsor: "Partenaires FLC legacy",
      effects: {
        morale: 4,
        rep: 7,
        hype: 10,
        stats: move.offset > 0
          ? { power: 1, chin: 1, cardio: -1, discipline: 1 }
          : { cardio: 1, iq: 1, power: -1, discipline: 1 },
      },
      weightId: weight.id,
      weightLabel: weight.label,
      weightMoveOffset: move.offset,
      doubleCategory: true,
    };
  }

			  function buildContractOffers(career = ui.career) {
			    const status = promotionStatus(career);
			    if (status.activeContractBlock && !status.promotionEligible && !status.doubleCategoryEligible) return [];
	    ...
	    const offers = [
	      {
	        ...
	        titleClause: debtTrouble ? "Objectif: 3 victoires propres, 0 forfait, 0 blessure majeure" : career.rank <= 5 || status.hasBelt ? "Objectif: prochain combat a forte valeur sportive" : "Objectif: saison gagnante pour grimper au ranking",
	    ...
	    if (status.doubleCategoryEligible) {
	      status.doubleCategoryMoves.forEach(move => {
	        offers.push(doubleCategoryContractOffer(career, move, status));
	      });
	    }
	    if (sponsorEligible) {
	      offers.push({
	        ...
	        titleClause: premiumSponsor ? "Objectif: obtenir un main event pour bonus sponsor" : "Objectif: 2 combats puis renegociation libre",
	    ...
	    return offers.map(normalizeContractOffer);
	  }
	  ...
	  function contractOffersNeedRefresh(career) {
	    ...
	    const hasStaleObjective = offers.some(offer => offer.titleClause !== clearContractObjectiveText(offer.titleClause));
	    if (hasStaleObjective) return true;
	    if (hasTrustOffer && !status.debtTrouble) return true;
	    if (status.doubleCategoryEligible) {
	      const expectedDoubleIds = (status.doubleCategoryMoves || []).map(move => `flc-double-${move.offset > 0 ? "up" : "down"}`);
	      if (expectedDoubleIds.some(id => !offers.some(offer => offer.id === id))) return true;
	    }
  ...
  function unlockLegendMode(career) {
    career.flags = career.flags || {};
    const alreadyUnlocked = Boolean(career.flags.legendModeUnlocked);
    career.flags.legendMode = true;
    career.flags.legendModeUnlocked = true;
    career.potential = Math.max(career.potential || 0, LEGEND_STAT_CAP);
    if (career.tier < LEGEND_TIER) {
      const legendOrg = orgForTier(LEGEND_TIER);
      career.tier = LEGEND_TIER;
      career.org = { id: legendOrg.id, label: legendOrg.label, org: legendOrg.tier, summary: legendOrg.summary, stats: {} };
      career.rank = 1;
      if (career.contract && Number(career.contract.tier || 0) < LEGEND_TIER) {
        career.contract = {
          ...career.contract,
          org: legendOrg.label,
          orgId: legendOrg.id,
          ...
          titleClause: "Objectif: battre des legendes et marquer le Pantheon",
          sponsor: "Partenaires Hall of Fame",
          purseBoost: Math.max(career.contract.purseBoost || 1, 1.35),
        };
      }
    }
    if (alreadyUnlocked) return;
    career.moments.push("Vous etes desormais une Legende de l'octogone: nouveau panel d'adversaires et de defis.");
    addNews(
      career,
      "Legende de l'octogone",
      `${career.name} est desormais une legende de l'octogone. Un nouveau panel de combattants et de defis est disponible, avec des stats qui peuvent depasser 100.`,
      "good"
    );
  }
  ...
  function chooseContract(index) {
    ...
    const oldWeight = career.weight;
    if (offer.doubleCategory) {
      tagActiveCurrentTitlesWithWeight(career);
      career.flags.doublePath = true;
      career.flags.doubleCategoryFrom = oldWeight?.label || "";
    }
    career.tier = offer.tier;
    career.org = { id: offer.orgId || org.id, label: org.label, org: org.tier, summary: org.summary, stats: {} };
    if (offer.weightId) {
      const nextWeight = getById(WEIGHTS, offer.weightId);
      if (nextWeight) {
        career.weight = nextWeight;
        career.flags.doubleCategoryTo = nextWeight.label;
      }
    }
    ...
    career.history.push({
      ...
      text: `${offer.label}. Prime: ${formatMoney(offer.money)}.`,
	    ...
	    showDecisionResult(career, {
	      title: offer.label,
	      text: offer.doubleCategory
	        ? `${career.name} change de categorie pour viser une deuxieme ceinture FLC en ${career.weight.label}. La carriere precedente reste au palmares.`
	        : `${career.name} signe avec ${org.label}: ${formatCombats(offer.fights)}, prime ${formatMoney(offer.money)}, ${offer.titleClause}.`,
	      effects: contractEffects,
	      visual: contractSigningVisualKey(career, `${offer.id}-${offer.weightId || offer.tier}`),
    ...
    `;
		  ...
		  function updateVisualParallax() {
		    ...
		    visualCards.forEach(card => {
		      ...
		      const maxParallax = priorityChoice ? (isMobile ? 118 : 92) : (isMobile ? 56 : 42);
		      const maxFlowLift = priorityChoice ? (isMobile ? 320 : 230) : (isMobile ? 120 : 84);
		      const flowFactor = priorityChoice ? (isMobile ? 0.95 : 0.68) : (isMobile ? 0.34 : 0.24);
		      if (reducedMotion) {
		      ...
		      const parallax = clamp(centerOffset / window.innerHeight * (priorityChoice ? 78 : 42), -maxParallax, maxParallax);
  ...
  function renderRankingPanel(career) {
    ...
    return `
	      <div class="contract-panel">
	        ...
	        <div class="contract-row">
	          <span>${iconOnly("target", "O")} Objectif contrat</span>
	          <strong>${esc(clearContractObjectiveText(contract.titleClause))}</strong>
	        </div>
	        ${clauseProgress ? `
	          <div class="contract-row">
	            <span>${iconOnly(contenderClauseReady(career) ? "circle-check" : "target", "C")} Progression objectif</span>
  ...
  function renderPromotionPath(career) {
    ...
    const clauseText = career.contract?.contenderWinsRequired
      ? `${Math.min(career.contract.contenderWins || 0, career.contract.contenderWinsRequired)}/${career.contract.contenderWinsRequired} victoires d'objectif`
      : "pas d'objectif a victoires";
    const rows = [
      {
        ok: status.legendStep
          ? status.ufcDominance
          : status.ufcStep
            ? status.internationalDominance || status.ufcBreakout || status.hasBelt
            : status.internationalStep
              ? status.nationalBreakout || status.strongSeason
              : status.localBreakout || status.cageSuccess,
	        label: status.legendStep ? "Domination FLC" : status.ufcStep ? "Resultats KSW/PFL" : status.internationalStep ? "Resultats National" : "Resultats cage",
	        value: status.legendStep
	          ? `${status.ufcWins} victoires FLC, ${status.doubleCategoryEligible ? "2e categorie ouverte" : "domination a prouver"}`
          : status.ufcStep && (status.internationalDominance || status.ufcBreakout) ? "FLC surveille le bilan"
          : status.internationalStep && status.nationalBreakout ? "KSW/PFL surveille le bilan"
          : status.perfectSeason ? `saison parfaite ${status.wins}/${status.honored}` : status.hasBelt ? "ceinture active" : `${status.wins} victoire(s), serie ${career.streak}, rang #${career.rank}`,
      },
      {
        ok: status.legendStep ? status.legendOvrReady || status.doubleCategoryEligible : status.ufcStep ? status.internationalDominance || status.ufcBreakout || status.businessSuccess : status.internationalStep ? status.nationalBreakout || status.businessSuccess : status.businessSuccess,
        label: status.legendStep ? "Double ceinture" : status.ufcStep ? "Seuil FLC" : status.internationalStep ? "Seuil KSW/PFL" : "Hype et business",
        value: status.legendStep
          ? status.doubleChampionReady ? "2 ceintures FLC validees" : `OVR ${overall(career)} | ${status.flcTitleCategories}/2 ceintures`
          : status.ufcStep ? "saison forte, top 5 ou 3 victoires"
          : status.internationalStep ? "3 victoires propres ou invaincu"
      ...
      {
        ok: status.legendStep ? status.legendEligible || status.doubleCategoryEligible : status.visibilityAccess || status.clauseReady || status.nationalBreakout || status.internationalDominance,
        ...
        value: status.legendStep
          ? status.legendEligible ? "statut Legende pret" : status.doubleCategoryEligible ? "monter/descendre au bilan" : `ceintures FLC ${status.flcTitleCategories}/2`
          : status.ufcStep
          ? status.promotionEligible ? "offre FLC au bilan" : status.hasBelt ? "champion KSW/PFL" : "saison forte ou top 5"
          : status.internationalStep
            ? status.promotionEligible ? "KSW/PFL sur la table" : "3 victoires propres"
            : status.clauseReady ? "objectif valide" : clauseText,
      },
    ];
    return `
      <div class="promotion-path ${status.promotionEligible || status.doubleCategoryEligible ? "ready" : ""}">
        <strong>${iconOnly(status.promotionEligible || status.doubleCategoryEligible ? "circle-check" : "move-up-right", "P")} ${status.legendStep ? "Route double champion" : status.internationalStep ? "Signature internationale" : `Montee vers ${esc(status.targetLabel)}`}</strong>
	        <span>${status.promotionEligible ? `Offre ${esc(status.targetLabel)} possible au prochain bilan.` : status.doubleCategoryEligible ? "Domination FLC validee: tu peux monter ou descendre de categorie pour viser une deuxieme ceinture." : status.legendStep ? "Pour devenir Legende: dominer le FLC, changer de categorie, puis gagner une deuxieme ceinture." : status.ufcStep ? "Le FLC peut contacter un champion KSW/PFL, une saison forte, un top 5 ou une serie de 3 victoires sans forfait." : status.internationalStep ? "Au National, 3 victoires propres ou une saison invaincue peuvent declencher KSW/PFL." : "Une grosse saison peut suffire: victoires, image minimale et dossier propre."}</span>
      ...
      <div class="choice-grid contract-offer-grid mobile-option-stack visual-embedded-choice-grid contract-visual-choice-grid">
        ${career.pendingContracts.map((offer, index) => `
          <button class="choice-btn contract-offer-card" data-action="choose-contract" data-index="${index}">
            ...
            <small>${esc(offer.tag || "Contrat")} | Sponsor: ${esc(offer.sponsor)}. ${esc(clearContractObjectiveText(offer.titleClause))}</small>
          </button>
        `).join("")}
      </div>
    `;
      ...
      <div class="contract-offers-priority visual-choice-priority">
        <div class="contract-signing-visual">
          ${renderVisualResultCard(
            ...
            "Les offres sont sur la table. Choisis le cadre de ta prochaine annee: organisation, nombre de combats, prime et objectif sportif.",
            ""
          )}
        </div>
	            ...
	            `).join("")}
		          ...
		          ${mobileSwipeDeck(opportunityChoices.map(option => ({
		            ...
		            leftIntent: "Je n'y vais pas",
			            ...
			            summary: campOpportunitySwipeSummary(opportunity, career),
	  ...
	  function onlineCareerSyncKey(career) {
	    ...
	    return JSON.stringify({
	      ...
	      titles: (career.titles || []).map(title => ({
	        ...
	        weightId: title.weightId || "",
	        weightLabel: title.weightLabel || "",
	        lost: Boolean(title.lost),
	      })),
	      stats: career.stats || {},
	      money: career.money || 0,
